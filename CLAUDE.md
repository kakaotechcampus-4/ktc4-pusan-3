...
### PR 쓰는 법 — 팀 내부 PR · 멘토 PR 공통 (1주차 멘토 피드백, 2026-09-10)
...
- **질문 하나에 결정 하나.** "이 방식이 괜찮을까요?" 가 아니라 **"A 로 하려고 하는데 B 조건에서 문제가 없는지"** 로 쓴다. 서로 다른 결정이 한 질문에 섞이면 답하는 쪽이 쪼개서 되묻는다 (1주차 인증 질문이 4개로 쪼개졌다).
...
- **재리뷰 주기.** 멘토 리뷰 → 피드백 반영은 각자 `refactor/<part>-…` 브랜치로 `develop` 에 → 토 10:00 까지 PR 본문 "지난 리뷰 반영" 을 채우고 재요청 → approve 후 일요일 `main` 병합. **그 사이 신규 기능 브랜치는 `develop` 에 합치지 않는다** (다음 주 수요일 PR 로). 합치면 재리뷰 범위가 섞이고 충돌이 난다.
