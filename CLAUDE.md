...
- 🚨 **키·몸무게는 잰 날짜가 달린 기록으로 둔다.** 추천을 만드는 데 쓸 수 있고, 그때는 외부 LLM 으로 보낼 수 있다 — 그 값이 필요한 Agent 에만 넘긴다 (위 "Agent 별 필요 필드만"). 아이 자신의 기록을 시간순으로 보여주는 그래프는 만들 수 있다 — 보호자가 넣은 숫자를 그대로 그리는 것이다. 다만 화면과 추천 결과 어디에도 **백분위·또래 비교·표준 성장곡선 겹치기·"빠르다/느리다/정상" 같은 평가 표현(색 포함)** 을 만들지 않는다 — 발달 평가로 읽힌다 (§1 안 만드는 것).
...
- **키·몸무게를 어느 Agent 가 어떤 추천에 쓰는지** (#75) — 외부 LLM 으로 보낼 수 있다는 것까지는 정했다 (개인정보 처리방침에 전달 항목으로 적는다). 실제로 쓰는 Agent 가 생기면 여기서 빼고 `docs/` 에 고정한다
