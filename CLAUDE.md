...
- 🚨 **키·몸무게는 잰 날짜가 달린 기록으로 둔다.** 추천을 만드는 데 쓸 수 있고, 그때는 외부 LLM 으로 보낼 수 있다 — 그 값이 필요한 Agent 에만 넘긴다 (위 "Agent 별 필요 필드만"). 다만 화면과 추천 결과 어디에도 성장 차트·백분위·또래 비교·증감 표시를 만들지 않는다 — 발달 평가로 읽힌다 (§1 안 만드는 것).
