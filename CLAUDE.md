...
- 🚨 **키·몸무게는 잰 날짜가 달린 기록으로만 둔다.** 성장 차트·백분위·또래 비교·증감 표시를 만들지 않는다 — 발달 평가로 읽힌다 (§1 안 만드는 것). 추천에 쓸지, 외부 LLM 으로 보낼지는 정해지기 전까지 **보내지 않는다** (§10).
...
- **아이의 성별·키·몸무게를 추천에 쓰는지, 외부 LLM 으로 보내는지** (#75) — 정해지기 전까지는 공통 컨텍스트에 넣지 않는다
