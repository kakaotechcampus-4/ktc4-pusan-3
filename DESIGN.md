...
**The 도메인 색은 출처다 규칙.** 도메인 색이 말하는 것은 "어느 Agent 의 결과인가" 하나다. 그래서 **Agent 가 만지지 못하는 데이터에는 도메인 색이 붙지 않는다** — 11 의 알레르기·건강 기록은 보호자만 확정할 수 있어서 `health` 플럼을 쓰면 화면이 규칙의 반대말을 한다. 그 화면은 도메인 4색을 하나도 쓰지 않고, 색은 승인 게이트(`caution`)와 회수(`danger`) 두 자리에만 선다.
...
**UI 라이브러리를 쓰지 않는다.** 토큰과 1:1 로 붙고 고치기 쉬운 쪽을 골랐다. 예외는 둘뿐이다 — 바텀시트는 네이티브 `<dialog>` 위에(포커스 트랩·ESC·바깥 `inert`·스크림을 브라우저가 준다), 달력은 `react-day-picker` 위에(그리드 ARIA·방향키·월 경계·로케일). 둘 다 자기 디자인을 들고 오지 않는다. 🚨 **얹는 범위는 자리마다 다르다** — 09 월 그리드는 라이브러리 머리줄을 그대로 쓰고 `date-field` 는 머리줄만 직접 그린다(`hideNavigation`). 그 넷은 어느 쪽에서도 가져오지 않는다 — 넷이 얹은 이유 전부라, 하나라도 직접 짜면 얹을 이유가 없어진다.
...
- **Primary:** `brand` 배경 + 흰 글자. 🚨 **한 화면에 하나.** 두 개면 무엇이 다음 행동인지 부모가 판단해야 한다. 예외는 **목록 항목**(05 제안 카드의 "이걸로") — 이때 화면의 다른 곳에는 primary 가 없어야 한다. 🚨 **그 "한 화면" 을 가르는 선은 인라인이냐 모달이냐다** — 바텀시트는 뒤가 `inert` 라 자기 primary 를 가져도 되지만, 줄 아래에 펼쳐지는 **인라인 확인 패널**은 화면의 primary 와 같은 뷰포트에 함께 서므로 `secondary` 로 내려간다. 07 교정 확인이 `btn-primary` 인 것은 07 에 화면 primary 가 없어서고, 11 의 한 줄 지우기 확인이 `secondary` 인 것은 위에 저장 버튼이 서 있어서다
...
- **Danger:** `danger` 배경 + 흰 글자. **파괴적 확정에만** — 10 설정의 보호자 연결 끊기·탈퇴 확정·데이터 삭제, 11 의 알레르기·안전 정보 회수. 🚨 뜻으로 정의된 것이지 화면으로 정의된 것이 아니다(설정이 유일한 자리이던 시절의 서술이 오래 남아 있었다). 자리는 넓어져도 뜻은 그대로다 — 취소·거절·되돌릴 수 있는 삭제(11 측정 한 줄 지우기)에는 쓰지 않는다. 🚨 같은 이유로 **동의 철회가 이 목록에서 빠졌다** — 화면에서 끌 수 있는 동의는 되돌릴 수 있고 기록도 지워지지 않는다. 빨강을 칠하면 철회권을 돌려주려고 만든 구역이 철회를 말리는 것처럼 읽힌다
...
- **구역 머리줄 (섹션 라벨 · 건수 · 더하는 행동):** 라벨은 `label`/`brand` 다 — §2-2 가 브랜드 자리로 정해 둔 다섯 곳 중 하나이고, 안 쓰면 화면이 **회색 카드의 나열**이 된다. 라벨 오른쪽에 쌓인 건수(`chip-count`), 그 오른쪽에 더하는 행동(`btn-tertiary` compact). 🚨 **더하는 버튼을 목록 아래 전체 폭 `secondary` 로 두지 않는다** — `surface` 바탕 + 테두리라 목록 상자와 실루엣이 같아서 둥근 사각형이 연달아 선다. 🚨 그 줄에 `min-h-touch` 를 깔지 않는다(버튼 없는 구역까지 44 가 되면 라벨과 설명이 떨어진다). 🚨 건수가 0 이면 안 그린다 — `empty-state` 가 같은 숫자를 더 크게 말한다.
- **목록에 더하고 고치는 행동:** 구역 머리줄의 더하기와 목록 줄의 고치기·내리기는 **글자 버튼이 아니라 `icon-button`** 이다. 글자 버튼을 두면 13px 섹션 라벨 옆에서 제일 무거운 것이 버튼이 되고, 구역이 여럿이면 같은 상자가 화면에 줄줄이 선다. 🚨 뜻은 `IconButton` 의 `label`(스크린리더 이름 + 툴팁)과, 눌러서 열리는 시트·패널의 글자가 진다 — 아이콘만으로 뜻을 나르지 않는다. 🚨 같은 아이콘이 한 화면에 둘이면 `label` 에 **어느 구역인지**까지 담는다. 🚨 "내리기" 에 `Trash2` 를 쓰지 않는다: 지우는 게 아니라 내리는 것이고 행은 남는다.
- **읽기 카드 + 고치는 시트:** 거의 안 고치는 값(아이 별명·생일·성별)은 **입력칸을 늘 열어 두지 않는다.** 평소에는 읽는 `card` 한 장(위: `icon-tile` + 이름 `title` + 나이, 아래: 선으로 갈린 `dl` 두 줄 · 값은 오른쪽 정렬), 고치는 것은 구역 머리줄의 `icon-button`(`Pencil`)이 여는 **바텀시트**가 받는다. 드문 일을 위한 입력이 늘 펼쳐져 있으면 자주 보는 것이 그만큼 아래로 밀리고, 화면이 "아이" 가 아니라 "설정 폼" 으로 읽힌다. 🚨 이 시트는 승인 게이트가 아니다(되돌릴 수 있는 값) — `caution`·`btn-approve` 를 쓰지 않고 스크림 탭으로 닫힌다.
- **날짜 표기가 자리마다 다르다:** 최근 날짜를 훑는 목록(측정 기록·캘린더)은 `formatDay`(**월·일·요일**, 연도 없음), 생일처럼 **해가 중요한 값**은 `formatDateWithYear`(**연도·월·일**, 요일 없음). 🚨 생일에 목록용 표기를 쓰면 "4월 2일 (금)" 이 떠서 어느 해인지가 사라진다.
- **폼 구역:** 입력칸은 그 자체가 테두리 있는 면이라 **카드로 감싸지 않는다**(상자 안의 상자). 한 화면에 폼 구역과 목록 구역이 같이 서면 **담김의 차이**가 구역의 리듬을 만든다 — 폼은 `canvas` 위, 목록만 상자.
- **Date field:** 값을 보여주는 트리거(입력과 같은 사양) + 달력을 담은 **바텀시트**. 오늘 이후 비활성 · 요일과 월 이름은 한국어. 머리줄은 `‹ 2021년 4월 ⌄ ›` 한 줄이고 **연월을 누르면 연도 그리드(4열 · 최근이 위) → 달 그리드(3열 · 범위 밖 잠금)** 로 펼친다 — 생일은 만 4세면 48개월을 거슬러야 해서 화살표만으로는 48번이다. 🚨 라이브러리의 `captionLayout="dropdown"` 을 쓰지 않는다: 그건 네이티브 `<select>` 두 개이고 이 시스템은 이미 그걸 안 쓰기로 했다(달력 안이라고 예외를 두지 않는다). 🚨 화살표는 날짜 면에서만 서고 감출 때는 `invisible`(포커스·a11y 트리에서 함께 빠진다 · `opacity-0` 는 보이지 않는데 탭으로 닿는다). 🚨 그리드가 열리면 고른 칸으로 포커스가 들어가고 달을 고르면 연월 버튼으로 돌아온다(안 옮기면 `<body>` 로 떨어진다). 🚨 쉐브론을 회전시키지 않고 아이콘을 갈아 끼운다. 🚨 세 면의 높이를 날짜 면에 `rem` 으로 맞춘다 — 안 그러면 시트 윗변이 448 ↔ 548px 로 뛴다
...
**하단 네비**는 4칸(홈 · 캘린더 · 기록/기억 · 아이)이다. 아이콘 20 + `caption` 라벨을 세로로 쌓는다. 앞의 셋이 "무슨 일이 있었나" 라면 네 번째 "아이" 칸은 **아이 자체**로 간다(11 프로필). 그 칸의 아이콘은 `Baby` 가 아니라 `UserRound` 다 — 대상 연령이 유아라 아기 아이콘은 사실과 다르고, "원색 캐릭터 키즈 앱으로 가지 않는다" 와 같은 방향으로 어긋난다. 붙는 자리는 가는 곳 **네 화면 + 설정**이다. `touch`(44)는 하한이고 실제 높이는 내용이 정한다(51). **바 전체가 `surface-muted` 이고 위에 선은 없다** — 바닥은 선이 아니라 면으로 만든다. 활성 `brand` 아이콘·라벨 + 칸 위쪽 `brand` 2px, 비활성 `ink-subtle`, press 는 `surface`(바닥보다 밝은 쪽)이고 **호버는 없다**(대부분 웹뷰라 호버가 없고, 마우스에서만 칸이 밝아지면 바닥이 들썩인다). 🚨 `line` 1px 은 canvas 위에서 1.21:1 이고 03 홈에는 같은 선이 27px 위에도 있어서, 선을 더 그으면 하단이 줄 쳐진 슬래브 하나로 읽힌다. 🚨 활성 표시를 색 하나에 맡기지 않는다 — `brand` 와 `ink-subtle` 은 휘도 차 1.15:1 이고 본문 서체가 단일 웨이트라 굵기로도 못 만든다. 🚨 흐름 중인 화면(04 저장 결과 · 05 제안 후보 · 온보딩 · 로그인)에는 붙이지 않는다 — 고르는 도중에 새는 길을 만들면 그 화면이 끝나지 않는다. 🚨 설정에서는 홈이 켜진 채로 두되 `aria-current` 는 뺀다 — 켜 보이는 것과 "지금 그 화면에 있다" 는 다른 신호다. 🚨 라벨을 지우지 않고, 아이콘은 lucide 다. 채팅바와 함께 붙을 때는 채팅바가 위 · 네비가 아래이고 safe area 는 네비만 받는다. 채팅바 구역은 위 8 / 아래 20 으로 **구역 사이가 구역 안보다 넓어야** 두 덩어리로 갈린다(고정 영역 192). 조립은 `Screen` 의 `bottomBar` · `nav` 가 소유한다.
...
- **아이콘 타일:** radius-field 정사각, 목록 줄 앞에. 톤 셋 — `brand`(`brand-soft` + `brand-ink`, 03·04) / `plain`(색을 호출자가 준다 — 07 기록·기억은 그 영역의 `{domain}-soft` + `{domain}-ink`) / `neutral`(`surface-muted` + `ink-muted` — `is_stale` 기억 카드, 그리고 11 의 두 줄 목록. 보호자가 적은 기록에는 출처 색이 없다). 🚨 07 의 타일을 브랜드 톤으로 바꾸지 않는다 — 초록 타일이 줄줄이 서면 브랜드가 장식이 되고 "어디서 왔나" 와 "무엇을 하는가" 가 같은 색이 된다
...
- **Don't** 병원·의료 앱처럼 보이게 만들지 않는다. 흰 바탕 + 파란 강조 + 차트·수치 중심은 이 제품이 **진단하지 않는다**는 규칙과 정면으로 어긋난다. 건강 도메인에도 심전도·청진기 기호를 쓰지 않는다. (그래프는 11-1 키 · 몸무게 **한 화면**에만 있고, 거기서도 백분위·또래 곡선·예상선을 그리지 않는다 — 디자인 시스템 §7 "변화 그래프".)
- **Don't** 원색 캐릭터 키즈 앱으로 가지 않는다. 채도 높은 원색 · 둥근 말풍선 · 마스코트를 넣지 않는다 — 이 화면을 여는 사람은 아이가 아니라 지친 보호자다.
- **Don't** SaaS 대시보드로 만들지 않는다. KPI 타일 · 지표 카드 그리드 · 스파크라인을 만들지 않는다. 부모가 자기 아이를 지표로 보게 하지 않는다. 🚨 **잰 값을 큰 숫자로 세우지 않는 것이 이 항목의 실제 경계선**이다 — 11-1 의 변화 그래프는 이 아이가 적은 값만 그리고 **또래·백분위와 견주지 않아서** 통과하지만, 같은 값을 `display` 크기 타일로 프로필에 올리면 그게 KPI 다.
...
- **Don't** 글자가 들어가는 요소의 높이를 고정하지 않는다. `min-h-*` 토큰으로만 쓰고, 말줄임으로 잘라 맞추지 않는다 — 문구가 가려진 승인 버튼은 승인 게이트가 아니다.
