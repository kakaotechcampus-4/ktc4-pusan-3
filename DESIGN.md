...
- **card-photo (08):** 4:3 이미지가 위, `line` 1px 아래로 그 사진에서 읽어낸 것이 이어 붙는 **한 덩어리**. 사진이 없을 때도 같은 4:3 칸이고 `surface-muted` 면에 브랜드 아이콘 하나다 — 점선 드롭존을 만들지 않는다(점선은 card-general 과 오래된 근거 칩이 이미 가져간 뜻이다). 확인 단계에서만 `accent` 이고 저장한 뒤에는 기본 테두리로 돌아온다. 🚨 **"승인 전에는 저장되지 않아요" 라벨에 `caution` 을 쓰지 않는다** — `caution` 은 승인 게이트 2곳 전용이고 08 의 저장은 `event` 를 draft 로만 만들어서 게이트가 아니다
