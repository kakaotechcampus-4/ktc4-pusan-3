...
- **Danger:** `danger` 배경 + 흰 글자. **파괴적 확정에만** — 10 설정의 동의 철회·데이터 삭제, 11 의 알레르기·안전 정보 회수. 🚨 뜻으로 정의된 것이지 화면으로 정의된 것이 아니다(설정이 유일한 자리이던 시절의 서술이 오래 남아 있었다). 자리는 넓어져도 뜻은 그대로다 — 취소·거절·되돌릴 수 있는 삭제(11 측정 한 줄 지우기)에는 쓰지 않는다
