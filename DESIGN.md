...
- **고르는 시트 (08 사진 출처):** 두 갈래를 고르는 자리라 내용이 **줄 목록**이다(아이콘 타일 + 라벨 + 한 줄 설명 + 쉐브론). 🚨 카드로 만들지 않는다 — 시트 안에서 카드 안의 카드가 된다. 🚨 **화면을 새로 만들지 않고 시트로 끝내는 기준**은 "고르고 나면 곧바로 다음이 오는가" 다. 모든 시트에 닫는 버튼을 세운다(웹뷰에는 ESC 가 없다) — 아직 시작한 것이 없으면 "취소" 가 아니라 "닫기" 다
