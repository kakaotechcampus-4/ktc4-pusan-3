...
class Settings(BaseSettings):
    ...
    def _require_kakao_keys_in_prod(self) -> "Settings":
        ...
        dev 를 뺀 이유는 따로 적어 둔다 — 아직 dev 환경이 존재하지 않는다(배포 도메인
        미정). 없는 환경의 규칙을 미리 정하면서 틀리는 쪽이 더 비싸다. 막아 두었는데
        dev 가 키 없이 쓰는 공용 서버였다면 팀이 가짜 키를 넣어 우회하고, 그 순간 이
        검증은 아무도 믿지 않는 규칙이 된다. 반대로 열어 두었는데 dev 가 운영 리허설
        자리였다면 키 누락이 prod 배포에서 잡힌다 — 늦지만 사용자 앞에 가기 전이다.

        🚨 dev 서버가 실제로 생기면 이 판단을 다시 한다. 조건에 "dev" 를 더하면 된다.
