        ...
        {options.map((option) => {
          ...
          return (
            <label
              key={option.value}
              className={cn(
                "group rounded-field min-h-field ease-standard flex flex-1 basis-0 cursor-pointer items-center justify-center gap-1.5 border px-3 py-2 text-center transition-colors duration-120",
            ...
            >
              <input
                ...
                className="sr-only"
              />
              {/* 🚨 자리를 늘 차지하게 둔다 — 고를 때 아이콘이 생기면서 글자가 밀리면
                  "색만 바뀐다"(§8) 가 깨지고 칸이 들썩인다. */}
              <Check
                aria-hidden
                size={ICON_SIZE.sm}
                strokeWidth={ICON_STROKE}
                className={cn("shrink-0", selected ? null : "text-transparent")}
              />
