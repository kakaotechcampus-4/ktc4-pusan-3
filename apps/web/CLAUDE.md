...
- 🚨 **08 은 승인 게이트가 아니다.** `POST /photo-runs/{rid}/commit` 이 만드는 것은 **관찰**이고,
  일정은 `drafts` 로 내려와 보호자가 카드에서 넣는다 (#151) — 화면의 저장 버튼에 `btn-approve` 도
  `caution` 도 쓰지 않는다. 그 둘은 카드의 제출 버튼이 가져간다.
