...
| 요청 바디 | `{code}` 또는 `{access_token}` — **둘 중 하나만.** 지금은 `code` 만 구현 (§2-3) |
...
**계약서 변경 4건** — 요청 바디 정의, `expires_in` 추가, `POST /auth/logout` 신설, 에러 3건. **§01 공통 규약은 손대지 않는다** (§12).
...
| 4 | 세션 전달 | **헤더 (Bearer)** | 2번이 프론트라 서버 응답이 JSON 이다. 쿠키를 쓸 이유가 약하고 배포도 자유롭다 (§4-3) |

> **2번을 백엔드로 두면 3·4번이 따라 바뀐다.** 콜백이 백엔드면 응답이 리다이렉트라 JSON 을 실을 수 없어 세션을 쿠키로 내려야 하고, 그러면 CSRF 방어와 "웹·API 같은 사이트 배포" 제약이 함께 붙는다. 지금 안은 그 셋을 다 피한다.

---

## 1. 범위

**다루는 것** — 로그인·로그아웃 계약, `auth_identity` 생성 규칙, 세션 토큰 형식과 수명, 계정 스코프 동의 게이트와의 순서.
...
**단, 네이티브 SDK 는 다르다.** Android·iOS SDK 의 `loginWithKakaoTalk()` 은 토큰을 직접 준다. 그래서 §2-3 이 두 입력을 다 정의한다 — 지금 쓰지 않을 뿐 틀린 경로가 아니다.
...
```
① FE   Kakao.Auth.authorize({ redirectUri: <프론트 콜백>, state: <난수> })
        ↓
② 사용자  카카오 로그인 (세션이 살아 있으면 화면 없이 통과)
        ↓
③ 카카오 → 프론트 콜백 ?code=...&state=...
        ↓
...
**콜백을 백엔드로 두지 않은 이유** — 백엔드가 받으면 응답이 브라우저 리다이렉트가 되어 JSON 을 실을 수 없다. 세션을 쿠키로 내려야 하고, 그러면 CSRF 이중 제출과 "웹·API 같은 사이트 배포" 제약이 함께 붙는다. 프론트가 받으면 **계약서의 `POST /auth/{provider}` 모양과 Bearer 규약이 그대로 유지된다.**
...
### 2-3. 요청 바디는 두 입력을 정의하고 하나만 구현한다

```json
{ "code": "..." }            // 웹 · 웹뷰 — 서버가 client_secret 으로 교환
{ "access_token": "..." }    // 네이티브 SDK — app_id 대조 후 사용 (§7-2)
```

**정확히 하나만 허용한다.** 둘 다 오거나 둘 다 없으면 `400 validation_failed`.

지금은 `code` 만 구현한다. `access_token` 은 **네이티브 화면이 생길 때 여는 경로**다 — 계약서에 이미 있는 필드라 그때 계약서를 다시 고치지 않아도 된다.

> **네이티브 SDK 가 필요해지는 시점** — 앱은 웹뷰 껍데기라 지금은 웹과 같은 흐름을 탄다. 카카오톡 간편로그인도 웹뷰에서 `kakaotalk://` 스킴을 가로채면 될 가능성이 있다(§4-5, `M-02`). **그게 안 될 때만** 네이티브 SDK 를 붙이고 `access_token` 경로를 연다. 순서를 바꾸면 안 해도 될 일을 몇 주 한다.
...
이 줄은 **커버리지 규칙**이다("모든 엔드포인트가 인증을 요구한다"). 전송 방식을 못박은 것이지만, 이 문서가 `token` 을 불투명 문자열로 정의하는 것으로 그대로 성립한다. **§01 을 고치지 않는다.**
...
> **이 절(형식)과 §4-3(전달)의 결정은 나중에 바꿔도 싸다.** `session` 테이블과 토큰의 의미를 건드리지 않고 미들웨어가 읽는 위치만 바뀐다. 반대로 **§2 의 인증 방식은 비싸다.** 논의 시간은 그쪽에 쓰는 게 맞다.
...
| 만료 시 | `401 unauthenticated` → FE 가 `authorize()` 로 새 code 를 받아 `POST /auth/kakao` 를 다시 부른다 |
...
> ⚠️ **숨긴 iframe 으로 갱신하는 방식은 쓸 수 없다.** 브라우저의 서드파티 쿠키 차단(Safari ITP, Chrome) 때문에 iframe 안에서 카카오 쿠키가 실리지 않는다. **전체 페이지 리다이렉트만 신뢰할 수 있고, 그러면 화면이 한 번 전환된다.** 체감 속도는 `M-03` 으로 확인하고 12시간을 조정한다.
...
| 메모리 (모듈 변수) | XSS 에는 가장 안전하지만 **새로고침마다 재로그인 리다이렉트**가 걸린다 |
| `localStorage` | ✕ **금지.** XSS 한 건이 12시간짜리 토큰을 그대로 넘긴다 |

메모리 전용을 권하지 않는 이유 — 재로그인이 `kauth.kakao.com` 전체 페이지 리다이렉트를 타므로 새로고침마다 화면이 튄다. `sessionStorage` 는 그걸 막으면서 `localStorage` 보다 표면이 작다.
...
**다만 지금은 카카오 로그인이 앱 밖으로 튕긴다.** [`apps/mobile/src/config.ts`](../../apps/mobile/src/config.ts) 의 `isInternalUrl` 이 우리 origin 만 웹뷰에 두고 나머지를 시스템 브라우저로 넘긴다. `kauth.kakao.com` 이 다른 origin 이라 **로그인 페이지가 앱 밖에서 열리고 돌아오지 않는다.**
...
그 외 출처는 계속 시스템 브라우저로 넘긴다 — 웹뷰 안에서 임의 사이트가 열리면 그 스크립트가 같은 컨텍스트에 들어온다.

> **이게 되면 네이티브 SDK 가 필요 없다** (§2-3). 안 되면 네이티브 SDK 를 붙이고 `{access_token}` 경로를 열되, 토큰을 웹뷰로 넘기는 handoff 가 함께 필요해진다. `M-02` 로 먼저 확인한다.
...
노션 「보호자 권한과 동의」의 신설 테이블 목록에 없다. 토큰 형식을 이 문서에서 정하면서 필요해진 것이므로 **김명성님 확정 대상**이다.
...
### 7-2. `app_id` 대조 — 네이티브 경로에만 필요하다

`{access_token}` 입력(§2-3)을 여는 순간 **다른 앱이 발급받은 카카오 토큰을 우리 서버에 들이밀 수 있다.** 카카오 입장에서는 유효한 토큰이라 `access_token_info` 가 200 을 준다.
...
**이 검사가 없으면 아무 카카오 앱 개발자나 우리 서비스의 임의 계정으로 로그인할 수 있다.**

`{code}` 경로에는 필요 없다 — 우리 `client_secret` 으로 교환하므로 받은 토큰이 정의상 우리 앱 것이다. **네이티브 SDK 를 붙이는 날 이 절을 반드시 함께 구현한다.**
...
카카오 호출 실패를 로깅할 때 응답 본문을 통째로 찍으면 토큰이 섞여 들어간다. 상태 코드와 카카오 에러 코드만 남긴다. **에러 응답에 요청받은 값을 되돌려주지 않는다.**
...
불투명 토큰에는 **서명 키가 없다.** 토큰이 난수라 서명할 게 없다.
...
| 5 | **웹뷰 허용 목록**을 우리 도메인 + 카카오 인증 도메인으로 제한 (§4-5) | 웹뷰 안에서 임의 사이트가 열리면 그 스크립트가 같은 컨텍스트에 들어온다 |
...
계약서 §01 의 에러 형식을 그대로 쓴다. 아래 3개는 이 문서에서 **신설**한다.

| HTTP | code | 언제 | 신설 |
| --- | --- | --- | --- |
| 400 | `validation_failed` | `code` 와 `access_token` 이 둘 다 오거나 둘 다 없음 | |
| 401 | `unauthenticated` | 세션 토큰 없음·만료·이미 삭제됨 | |
| 401 | `invalid_provider_token` | 인가 코드 교환 실패(만료·재사용·불일치), 또는 **`access_token` 경로에서 app_id 불일치** | 🆕 |
...
`invalid_provider_token` 은 **실패 사유를 구분하지 않는다.** 구분해서 알려주면 "이 토큰은 다른 앱 것"이라는 정보를 공격자에게 준다. 초대 코드 실패를 `404 invalid_invite` 로 통일한 것과 같은 이유다.

`401 unauthenticated` 를 받은 FE 는 **재로그인 경로로 간다** (§4-3). `invalid_provider_token` 을 받으면 그 경로도 실패한 것이므로 실제 로그인 화면으로 보낸다 — 두 코드를 구분해 두는 이유가 이것이다.
...
| A-03 | `code` 와 `access_token` 을 둘 다 보냄 | `400 validation_failed` |
| A-04 | 둘 다 없이 빈 바디 | `400 validation_failed` |
| A-05 | 카카오가 코드 교환을 거절 (만료·재사용) | `401 invalid_provider_token`, `parent` **미생성** |
| A-06 | 🚨 **`access_token` 경로 + 다른 app_id** | `401 invalid_provider_token`, `parent` 미생성 (§7-2) |
| A-07 | 동의 전에 `POST /children` | `403 consent_required` |
| A-08 | 동의 전에 `GET /me` | **200** — 동의 화면이 부르는 경로 (§6-2) |
| A-09 | 만료된 세션 토큰으로 호출 | `401 unauthenticated` |
| A-10 | 🚨 **로그아웃 직후 같은 토큰으로 호출** | `401 unauthenticated` — **지연 없이 즉시** (§4-2) |
| A-11 | 카카오 API 가 500 | `502 oauth_provider_error`. 로그인 통과 경로 없음 |
| A-12 | 같은 회원번호로 동시 요청 2건 | `UNIQUE` 로 `auth_identity` 한 건만 생성, 둘 다 같은 `parent_id` |
| A-13 | `POST /auth/apple` | `422 validation_failed` |
| A-14 | 로그인 후 `session` 테이블 조회 | **응답 `token` 과 같은 문자열이 어디에도 없다** (해시 저장 확인) |
| A-15 | `parent.deleted_at` 이 찍힌 계정의 세션 | `401` 또는 `404 not_found` — 만료를 기다리지 않는다 |

**A-06 과 A-10 이 이 목록의 이유다.** A-06 은 네이티브 경로를 여는 날 반드시 켜야 하고(그전까지는 skip), A-10 이 실패하면 JWT 대신 불투명 토큰을 고른 이유가 사라진다.

카카오 호출은 유닛 층에서 **스텁으로 대체**한다. `POST /oauth/token` 과 `access_token_info` 두 응답 모양만 고정하면 A-01·A-02·A-05·A-06·A-11 이 검증된다.
...
| M-02 | 🚨 **웹뷰 안에서 카카오톡 간편로그인이 되는가** — 허용 목록 + `kakaotalk://` 스킴 가로채기 후 실제 기기에서 (§4-5) | 고태영 |
| M-03 | 카카오 세션이 살아 있을 때 **재로그인 리다이렉트 체감** → 12시간을 조정할지 판단 (§4-3) | 고태영 |

**M-02 가 이 문서에서 가장 중요한 확인 항목이다.** 되면 네이티브 SDK·handoff·`{access_token}` 경로가 전부 불필요하고, 안 되면 그 전부가 일정에 들어온다.
...
| 2 | **세션 수명 12시간이 맞는지** | `M-03` 결과에 달렸다. 리다이렉트 체감이 크면 늘려야 한다 | 고태영 |
| 3 | **네이티브 SDK 를 붙일지** | `M-02` 결과에 달렸다. 웹뷰에서 간편로그인이 되면 붙일 이유가 없다 | 고태영 · 김명성 |
...
KAKAO_REDIRECT_URI=      # 프론트 콜백 경로. 콘솔 등록값과 정확히 일치해야 한다 (운영·로컬 각각)
...
| 계약서 현재 | 바뀌어야 할 것 |
| --- | --- |
| `POST /auth/{provider}` 요청 `{access_token}` | **`{code}` 또는 `{access_token}` 중 하나**로 정의. 지금은 `code` 만 구현하고 `access_token` 은 네이티브 경로용으로 남긴다 (§2-3) |
| 응답 | **`expires_in` 추가.** 없으면 FE 가 만료를 미리 알 수 없다 (§3-1) |
