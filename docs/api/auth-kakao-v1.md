...
> **이 절의 결정은 나중에 바꿔도 싸다.** 토큰을 헤더로 보내느냐 쿠키로 보내느냐는 `session` 테이블과 토큰의 의미를 건드리지 않는다 — 미들웨어가 읽는 위치와 프론트가 저장하는 위치만 바뀐다. 반대로 **§2 의 카카오 인증 방식은 비싸다.** 인가 코드로 옮기면 엔드포인트 모양, 콘솔 설정, 모바일 흐름이 함께 바뀐다.
>
> 두 결정을 같은 무게로 다루지 말 것. 논의 시간은 §2 쪽에 쓰는 게 맞다.
