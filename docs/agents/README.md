...
**기피(polarity −1)는 제외 필터가 아니라 근거다.** "미끄럼틀을 싫어해서 이걸 골랐어요"가 되어야 한다. 후보에서 지워버리면 무엇을 피해 골랐는지 화면에 말할 수 없다. 기피 근거를 인용했으면 `reason`에 무엇을 피했는지 쓴다. 후보에서 지우는 것은 안전뿐이다 — `hazard_term`에 걸리는 것.
