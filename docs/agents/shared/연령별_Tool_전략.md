...
2. **가르는 방식은 도메인이 정한다.** 공통이 주는 것은 `life_stage()`의 월령까지다. `stage` 네 값은 참고값이고, 그걸 배타적 범주로 쓸지 `min_month` 눈금으로 쓸지 아예 tool을 안 여닫을지는 각 Agent가 자기 도메인을 보고 정한다. 아래는 지금까지 정한 네 가지다.
...
| 개월 | Food | Growth | Health | Activity | 공통 |
| --- | --- | --- | --- | --- | --- |
| **0** | 수유기 — 추천 없음 | 리듬 안내 · 보드북 · 성장 추이 | 진료 요약 · 복약 · 검진/접종 · 병원 · 전달 서류 | 놀이 추천 (닫힌 조합 없음) | affinity 없음 → 근거는 관찰(티어 3)뿐 |
| **4** | 이유기 시작 → `guide_weaning_stage` | – | – | – | |
| **12** | 유아식 → 급식·영양소 분석 전체 개방 / 꿀 금지 해제 | 교육 활동 · 자립·식사·전환 루틴 | – | – | |
| **18** | – | – | – | 관심 프로필 조회 개방 | **`profile_affinity` 생성 시작** → 티어 1·2 근거가 이때부터 존재 |
| **24** | – | 예절 연습 | – | – | |
| **36** | 섭취기준 1–2세 → **3–5세** | 습관 교정 · 누리과정 KB | – | 주변 장소 조회 개방 · 안전 차단 1단 | 안전 기준(작은 부품) 전환 |
| **48** | 질식 주의 식품 해제(설정값) | – | – | – | |
| **72** | – | – | 영유아 검진·국가예방접종 대상 종료 → 안내 문구 전환 | 안전 차단 2단 | v1 범위 상한 |

**18개월이 조용한 경계다.** Curator가 `profile_affinity`를 18개월부터 만들기 때문에 그 아래는 Food·Growth·Activity 모두 티어 1·2 근거가 **구조적으로 0**이다. 다만 **개인화가 아예 안 되는 구간은 아니다** — 티어 3(최근 14일 관찰)은 월령과 무관하게 살아 있어서, 프로필이 없어도 관찰이 쌓여 있으면 `kind="personalized"` 가 나간다. 관찰도 없는 진짜 초기만 일반 추천이고, 그 구간의 품질은 **일반 추천 템플릿과 문서 행(`*_doc`)의 품질**로 결정된다.
...
| 근거 모드 | 근거가 있으면 개인화 | 근거가 있으면 개인화 | 개인화 | 개인화 |

**근거 모드는 월령으로 안 가른다.** `rank_evidence()` 가 월령 인자를 받지 않는다 — 티어 1·2(`profile_affinity`)는 Curator 가 18개월부터 만들어서 그 아래는 구조적으로 0이지만, 티어 3(최근 14일 관찰)은 월령과 무관하다. 관찰이 있으면 `personalized`, 없으면 `general` 이다 (2026-09-23 결정).

어린 월령의 행동을 선호로 읽는 게 걱정이라면 막을 자리는 여기가 아니라 **Curator 승격 기준**이다 — 관찰을 근거로 인용하는 것과 성향으로 확정하는 것은 다르다. §10 A-3 과 같이 본다.
...
## 5-1. Activity — tool별 `min_month`

Growth와 같은 눈금 방식이다. 번호가 `5-1`인 이유는 §3·§4·§5·§9를 다른 문서 일곱 곳이 가리키고 있어서다 — 중간에 끼우면 그 링크가 전부 어긋난다.
...
18개월이 근거 모드를 가르지는 않는다. 그 아래는 `profile_affinity`가 없어 티어 1·2가 0행일 뿐이고, 관찰이 있으면 `kind="personalized"`가 나간다 (§4 표 아래).
...
| 17개월 / 18개월 | 둘 다 추천이 나가고, 근거 모드는 월령이 아니라 **근거 유무**가 정한다 — 관찰 0건이면 양쪽 다 `general`, 관찰이 있으면 17개월도 `personalized`. 18개월부터 티어 1·2(affinity)가 근거에 얹힐 수 있을 뿐이다 |
...
| A-3 | Curator의 affinity 생성 최소 월령이 정말 18개월인지 · 도메인별로 다른지. **열어 둔다** — Activity 문서의 18개월은 활동 도메인 게이트 근거(K-DST가 18개월부터 자조 영역을 추가)지 Curator 승격 기준이 아니고, 대조할 Curator 코드도 아직 없다. 어린 월령의 단순 반사 행동을 성향으로 굳히지 않는 문제도 여기서 본다 — 추천 근거 쪽은 월령으로 막지 않기로 했다(§4) |
