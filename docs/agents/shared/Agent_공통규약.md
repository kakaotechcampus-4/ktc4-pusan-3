...
| +1 / candidate | 근거(티어2) | **안 씀** | 근거(티어2) |
| **−1 / candidate** | **근거(티어2)** | **안 씀** | **근거(티어2)** |
| NULL | 안 씀 → 역질의 후보 | 안 씀 | 안 씀 → 역질의 후보 |
| 관찰(티어 3) | 쓴다 | **안 쓴다** (id도 반환 안 함) | 쓴다 |
...
- 18개월 미만은 `profile_affinity`가 구조적으로 0행이다 → 정상 경로로 테스트한다.
...
| 규칙 | 값 |
| --- | --- |
| run당 모델 호출 | **최대 5** (Supervisor 1 + Memory 2 + 도메인 2). 아래 재호출 예외 포함 시 최대 7 |
