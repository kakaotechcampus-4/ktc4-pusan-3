...
세 도메인이 같다. `rank_evidence()` 와 `pick_followup()` 은 도메인 인자를 받지 않아서 애초에 갈릴 수 없다 — Activity 만 달랐던 앞 판은 9/20 회의 결론과 `suggestion_evidence` 의 허용 `source_kind` 양쪽과 안 맞았다(2026-09-23 정정). **Health 만 이 표의 대상이 아니다** — 맨 아래 항목을 볼 것.
