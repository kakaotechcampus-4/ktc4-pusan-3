...
| `--color-surface` | `#FFFFFF` | 카드·시트·입력. 바탕 위에서 1.05:1 로만 뜬다 |
...
| `--color-danger` / `-hover` / `-soft` / `-ink` | `#B3261E` (6.2:1) / `#951F19` (흰 글자 8.4:1) / `#FAE8E5` / `#8F1D17` (7.5:1) | 🚨 **알레르기 경고 · 건강 추천 중단 · 파괴적 확정 전용** |
...
⚠️ **가운뎃점(`·`)과 줄표(`—`)는 예외다.** 뜻을 나르는 기호가 아니라 한국어 문장부호라서 그대로 쓴다 ("별명 · 생일 · 관계"). 코드 주석·문서의 🚨 도 화면에 안 나가므로 대상이 아니다.
